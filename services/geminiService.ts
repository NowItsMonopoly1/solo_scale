...
export class GeminiService {
  ...
  public static getInstance(config: SystemConfig): GeminiService {
  ...
  public updateConfig(config: SystemConfig) {
    this.config = config;
  }
   ...
   */
  public async chatWithSpeedAgent(history: { role: string; parts: { text: string }[] }[], userMessage: string) {
  ...
  public async extractDocumentData(
    ...
    const schema = (schemas as any)[documentType] || schemas.other;
...
- Quality issues are LOW severity`;
    ...
    try {
      ...
      return {
        ...
        discrepancy_alerts: [{
          field: "extraction",
          issue: "Failed to extract data from document: " + (error as Error).message,
          severity: "high"
        }],
   ...
   */
  public async analyzeLeadUrgency(lead: Lead): Promise<{ score: number; analysis: string }> {
    const ai = this.ai;
    const prompt = `
